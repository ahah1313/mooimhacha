...
```
> 발화 전송은 WebSocket `utterance:new` 전용. REST 엔드포인트는 두지 않는다.
...
POST   /api/action-items/:id/attachments   # 자료 첨부 (업로드 방식 미정 — [09](09-미결정-사항.md))
GET    /api/action-items/:id/attachments
